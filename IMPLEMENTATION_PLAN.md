...
1. **LSTM 모델 학습 시간**: 충분한 데이터 수집 및 학습 시간 필요

   - 대응: 3일차에 데이터 수집 도구를 먼저 구현하고, 간단한 제스처부터 시작

2. **제스처 인식 정확도**: 실시간 인식 정확도가 낮을 수 있음

   - 대응: 임계값 조절 기능 제공, 신뢰도 기반 필터링

3. **크로스 플랫폼 호환성**: PyAutoGui 동작이 OS별로 다를 수 있음

   - 대응: OS별 테스트 및 대체 방법 준비

4. **웹캠 접근 권한**: macOS에서 웹캠 권한 요청 필요

   - 대응: 명확한 에러 메시지 및 권한 요청 가이드

## 개발 전략

### UI 우선 접근법

- UI를 먼저 구현하여 전체적인 구조와 흐름을 명확히 함
- 기능을 단계적으로 UI에 연결하여 점진적 개발
- 각 단계마다 동작하는 버전을 유지

### 공통 기능 우선 구현

- 공통 필수 기능(동작 감지 시작/종료)을 먼저 완성
- 이후 모드별 기능을 추가하는 방식으로 확장
- 기본 기능이 안정화된 후 고급 기능 추가
...
3. 공통 기능: 동작 감지 시작/종료
4. 기본 제스처 인식 (시작, 종료)
5. PPT 모드 기본 기능 (다음, 이전)
6. 유투브 모드 기본 기능 (재생, 일시정지)

### 선택 기능 (시간 여유 시)

1. 유투브 모드 고급 기능 (볼륨, 음소거, 전체화면)
2. PPT 슬라이드 쇼 시작 기능
3. 제스처 시각화 고도화
4. 설정 다이얼로그
5. LSTM 모델 고도화
...
- 매일 저녁 일일 회고 및 다음 날 계획 조정
- 버그나 이슈 발견 시 즉시 기록 및 우선순위 결정
- 팀원 간 작업 분담 시 이 계획을 기준으로 할당
